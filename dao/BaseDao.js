class BaseDao {
  ...
  async findAll(condition, constraints) {
    try {
      ...
      debugger
      let [err, data] = this.Model.find(condition, constraints ? constraints : null).then(data => [null, data] ).catch(err => [err, null]);
      debugger
      return data;
    } catch (error) {
  ...
  async aggregate(condition) {
    try {
      let data = await this.Model.aggregate(condition);
      return data;
    } catch (error) {
      console.log('findAll error--> ', error);
      return error;
   ...
   * @returns {Promise}
   */
  async updateOne(condition, updater) {
    try {
   ...
   * @returns {Promise}
   */
  async findAndModify(condition, updater) {
    try {
      //this.Model.find(condition, constraints ? constraints : null).then(data => [null, data] ).catch(err => [err, null]);
      //let [err, data] = await this.Model.findOneAndUpdate(condition, updater).then(data => [null, data] ).catch(err => [err, null]);
      let result = await this.Model.findOneAndUpdate(condition, updater);
      return result;
    } catch (error) {
      console.log(`update error--> ${error}`);
      return error;
